...
| 담당 | 역할 |
|---|---|
| _(이름 추가)_ | TurtleBot3 SLAM/Nav2, 자율주행 순찰 로직, RealSense 카메라 스트리밍 |
| _(이름 추가)_ | STM32 임베디드 (지문인식 AS608, RFID 관리자 인증, WiFi 통신) |
| _(이름 추가)_ | 웹 대시보드, Flask 서버, YOLO 연동, DB 설계 |

---

## 향후 계획

- [ ] 분실물(가방) 클래스 YOLO 모델 학습 및 적용
- [ ] 자리비움 감지 기능
- [ ] 위급상황 발생 시 관리자 알림(문자/전화) 연동
- [ ] STM32 서보모터(출입문 제어) 연동
