...
- "조치 완료" 처리 시 자동으로 퇴실 처리(출결 기록 포함) + 출입문 자동 개방까지 한 번에 처리
   (지문 출결 전송용 TCP 채널을 양방향으로 확장해 구현, STM32 측 신호 수신은 인터럽트 기반으로 안정성 확보)
...
| 위급상황 문열림(DOOR_OPEN) 신호가 유실됨 | UART 폴링(`HAL_UART_Receive`) 방식은 서보모터 구동 등 블로킹 구간 중 하드웨어 수신 버퍼(1바이트)가 덮어써져 데이터 손실 | 인터럽트(`HAL_UART_Receive_IT`) + 링버퍼 방식으로 전환, 블로킹 구간에도 안전하게 수신 |
...
| _(고경민, 김재민, 고성훈)_ | TurtleBot3 SLAM/Nav2, 자율주행 순찰 로직(ROS2 노드화), RealSense 카메라 스트리밍 |
